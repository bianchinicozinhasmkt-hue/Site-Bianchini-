/**
 * R2 — acessibilidade de `#projetos` e regression check dos congelados.
 *
 * Derivado de `equipamentos-r1-2026-08-12/medicoes/a11y-e-regressao.mjs`. O alvo muda para
 * `#projetos` e **`#equipamentos` entra no conjunto congelado** — ela foi selada em R1, e
 * R2 é a primeira rodada que corre com ela fechada.
 *
...
async function goto(url) {
  ...
  await evaluate(`(() => {
    ...
    document.querySelectorAll('.reveal, .photo-mask, .line-mask').forEach((e) => e.classList.add('is-visible'));
...
resultado.regressao = await evaluate(`(() => {
  ...
  /* EQUIPAMENTOS — congelada em R1. As invariantes seladas lá: a vitrine é assimétrica
     (a fotografia prioritária domina e sangra à direita), zero card, e a protagonista
     vence a soma do apoio (S-04b). Aqui só se confere que continuam valendo. */
  const eq = document.getElementById('equipamentos');
  if (eq) {
    const rEq = eq.getBoundingClientRect();
    const imgs = [...eq.querySelectorAll('img')]
      .map((i) => { const r = i.getBoundingClientRect(); return { area: r.width * r.height, right: r.right }; })
      .filter((i) => i.area > 0)
      .sort((a, b) => b.area - a.area);
    const A = imgs[0]?.area ?? 0;
    const Btot = imgs.slice(1).reduce((s, i) => s + i.area, 0);
    out.equipamentos = {
      ...conta(eq),
      nImagens: imgs.length,
      A_sobre_Btotal: Btot ? +(A / Btot).toFixed(2) : null,
      protagonistaVence: Btot ? A > Btot : null,
      sangraDireita: imgs[0] ? Math.round(imgs[0].right) >= Math.round(rEq.right) - 1 : null,
    };
  }
...
if (r.equipamentos)
  console.log(
    `  EQUIPAMENTOS ${r.equipamentos.caracteres}c / ${r.equipamentos.blocos}b · ${r.equipamentos.nAmarelos} amarelos · ${r.equipamentos.h}px · ` +
      `${r.equipamentos.nImagens} imgs · A/Btotal=${r.equipamentos.A_sobre_Btotal} (${r.equipamentos.protagonistaVence ? 'protagonista vence' : 'FAIL S-04b'}) · ` +
      `sangra à direita=${r.equipamentos.sangraDireita}`,
  )
